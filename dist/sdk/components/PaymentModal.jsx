...
const PaymentModal = ({ onSubmit, amount, onClose }) => {
    ...
    const [estimatedTokenAmount, setEstimatedTokenAmount] = useState(null);
    ...
    useEffect(() => {
        if (wallet.connected && wallet.publicKey) {
            const fetchWalletTokens = () => __awaiter(void 0, void 0, void 0, function* () {
                setIsLoading(true);
                try {
                    const tokens = yield getFungibleTokensForWalletV2(wallet.publicKey.toString());
    ...
    useEffect(() => {
        const calculateRequiredAmount = () => __awaiter(void 0, void 0, void 0, function* () {
            ...
            try {
                ...
                const tokenPrice = yield JupiterService.getTokenPriceInUSDC(selectedTokenId);
    ...
    const handleSubmit = (wallet, walletAddress) => __awaiter(void 0, void 0, void 0, function* () {
        ...
        try {
            yield onSubmit({ fromToken, walletAddress, wallet, amount });
            toast.success("Payment completed successfully!", { id: toastId });
        }
        catch (error) {
            console.error("Payment submission error:", error);
            toast.error(`Payment failed: ${error instanceof Error ? error.message : "Unknown error"}`, { id: toastId });
        }
        finally {
            setIsSubmitting(false);
        }
    });
    return (<section className="bg-[#050510]/90 backdrop-blur-md min-h-screen flex items-center justify-center fixed w-full top-0 left-0">
      <Card className="w-full max-w-md p-6 rounded-xl shadow-2xl border border-purple-500/20 bg-[#0a0a1a] text-white relative overflow-hidden">
        ...
        <CardContent className="px-0 pb-0">
          <div className="grid grid-cols-[5fr_1fr] gap-3 items-top justify-start">
            <Select value={selectedTokenId} onValueChange={setSelectedTokenId}>
              <SelectTrigger className="w-full border border-gray-800 text-white bg-[#111125] h-12 rounded-lg focus:ring-purple-500 focus:border-purple-500 ">
                <SelectValue placeholder="Select token">
                  {selectedTokenId && getSelectedToken() ? (<div className="flex items-center">
                      <span>{(_a = getSelectedToken()) === null || _a === void 0 ? void 0 : _a.symbol}</span>
                    </div>) : ("Select token")}
                </SelectValue>
              </SelectTrigger>
              <SelectContent className="bg-[#111125] border border-gray-800 rounded-lg text-white">
                {isLoading ? (<div className="p-4 text-center">
                  ...
                  </div>) : walletTokens.length === 0 ? (<p className="p-4 text-center text-gray-400">
                    No tokens found
                  </p>) : (walletTokens.map((token) => (<SelectItem key={token.mint} value={token.mint} className="m-1 rounded hover:bg-[#1a1a30] focus:bg-[#1a1a30] focus:text-white">
            ...
            <div className="text-sm bg-[#111125] px-3 py-1 rounded-lg border border-gray-800 flex items-center justify-center h-12">
              {isCalculating ? (<div className="flex items-center space-x-1">
                  <Loader2 className="h-3 w-3 animate-spin text-purple-400"/>
                  <span className="text-gray-400">Calculating...</span>
                </div>) : estimatedTokenAmount ? (<div className="flex items-center space-x-1">
                  <span className="text-blue-400">{estimatedTokenAmount}</span>
                  <span className="text-gray-400">
                    {((_b = getSelectedToken()) === null || _b === void 0 ? void 0 : _b.symbol) || "tokens"}
                  </span>
        ...
        <footer className="mt-6">
          {!wallet.connected ? (<div className="flex justify-center">
              <WalletMultiButton />
            </div>) : (<Button className="w-full rounded-lg h-12 bg-gradient-to-r from-purple-600 to-blue-500 hover:from-purple-700 hover:to-blue-600 text-white font-medium transition-all duration-200 border-0" disabled={!wallet.connected ||
                !selectedTokenId ||
                !amount ||
                !toToken ||
          ...
          <div className="flex justify-between mt-4">
            <button onClick={onClose} className="text-xs text-gray-400 hover:text-gray-300 transition-colors">
              Cancel
            </button>
            <p className="text-xs text-gray-400">
